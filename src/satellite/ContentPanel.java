/*
 * To change this template, choose Tools | Templates
 * and open the template in the editor.
 */
package satellite;

import communication.CommunicationServer;
import java.awt.Color;
import java.awt.Dimension;
import java.util.ArrayList;
import java.util.HashMap;
import javax.swing.JCheckBox;
import javax.swing.JLabel;
import javax.swing.JOptionPane;
import javax.swing.JPanel;
import javax.swing.JTable;
import javax.swing.JTextArea;
import javax.swing.SwingConstants;
import javax.swing.SwingUtilities;
import utils.BusinessApplyTableModel;
import utils.BusinessRunningTableModel;
import utils.ButtonEditor;
import utils.ButtonRenderer;
import utils.Configuarator;
import utils.Constants;

/**
 *
 * @author shuai
 */
public class ContentPanel extends javax.swing.JPanel {

    private Configuarator conf;
    private Constants constant;
    private BusinessApplyTableModel businessApplyTM;
    private BusinessRunningTableModel businessRunningTM;
    private ArrayList<JLabel> siteLabels;
    private ArrayList<JCheckBox> siteCheckBoxes;
    private CommunicationServer communicationServer;

    /**
     * Creates new form ContentPanel
     */
    public ContentPanel() {
        initComponents();
        loadConf();

        siteLabels = new ArrayList<JLabel>();
        siteCheckBoxes = new ArrayList<JCheckBox>();

        communicationServer = new CommunicationServer(this);

        businessApplyTM = new BusinessApplyTableModel(this);
        tbBusinessApply.setModel(businessApplyTM);
        tbBusinessApply.getColumnModel().getColumn(6).setCellEditor(new ButtonEditor(this, communicationServer));
        tbBusinessApply.getColumnModel().getColumn(6).setCellRenderer(new ButtonRenderer());
        tbBusinessApply.getColumnModel().getColumn(7).setCellEditor(new ButtonEditor(this, communicationServer));
        tbBusinessApply.getColumnModel().getColumn(7).setCellRenderer(new ButtonRenderer());

        businessRunningTM = new BusinessRunningTableModel(this);
        tbBusinessRunning.setModel(businessRunningTM);
        tbBusinessRunning.getColumnModel().getColumn(9).setCellEditor(new ButtonEditor(this, communicationServer));
        tbBusinessRunning.getColumnModel().getColumn(9).setCellRenderer(new ButtonRenderer());
        tbBusinessRunning.getColumnModel().getColumn(10).setCellEditor(new ButtonEditor(this, communicationServer));
        tbBusinessRunning.getColumnModel().getColumn(10).setCellRenderer(new ButtonRenderer());

        addLabels();
    }

    private void loadConf() {
        conf = new Configuarator();
        constant = new Constants();
        constant.setSiteNum(Integer.parseInt(conf.getSiteNum()));
//        constant.setLoginPort(Integer.parseInt(conf.getLoginPort()));
//        constant.setMessagePort(Integer.parseInt(conf.getMessagePort()));
        constant.setPrivileges(conf.getPrivileges());
        constant.setBizBoardPort(Integer.parseInt(conf.getBusinessBoardPort()));
        constant.setBizBoardIP(conf.getBusinessBoardIP());
        constant.setRemoteIP(conf.getRemoteIP());
        constant.setRemotePort(Integer.parseInt(conf.getRemotePort()));
        constant.setLocalPort(Integer.parseInt(conf.getLocalPort()));

        HashMap<String, String> map = new HashMap<String, String>();
        for (int i = 0; i < constant.getSiteNum(); i++) {
            map.put("" + (i + 1), "");
        }
        constant.setSnrs(map);

    }

    private void addLabels() {
        int siteNum = Integer.parseInt(conf.getSiteNum());
        for (int i = 0; i < siteNum; i++) {
            JLabel lb = new JLabel("" + (i + 1));
            lb.setHorizontalAlignment(SwingConstants.CENTER);
            lb.setBackground(Color.red);
            lb.setOpaque(true);
            lb.setPreferredSize(new Dimension(30, 30));
            siteLabels.add(lb);
            plSites.add(lb);
        }
//        lbSite5.setBackground(new java.awt.Color(255, 0, 0));
//        lbSite5.setHorizontalAlignment(javax.swing.SwingConstants.CENTER);
//        lbSite5.setText("5");
//        lbSite5.setOpaque(true);
//        lbSite5.setPreferredSize(new java.awt.Dimension(30, 30));
//        plSites.add(lbSite5);
    }

    /**
     * This method is called from within the constructor to initialize the form.
     * WARNING: Do NOT modify this code. The content of this method is always
     * regenerated by the Form Editor.
     */
    @SuppressWarnings("unchecked")
    // <editor-fold defaultstate="collapsed" desc="Generated Code">//GEN-BEGIN:initComponents
    private void initComponents() {

        plSites = new javax.swing.JPanel();
        jPanel3 = new javax.swing.JPanel();
        jPanel2 = new javax.swing.JPanel();
        jScrollPane1 = new javax.swing.JScrollPane();
        taMessageRecorded = new javax.swing.JTextArea();
        jScrollPane2 = new javax.swing.JScrollPane();
        taMessageSending = new javax.swing.JTextArea();
        plSitesConnected = new javax.swing.JPanel();
        jPanel8 = new javax.swing.JPanel();
        jLabel5 = new javax.swing.JLabel();
        jButton1 = new javax.swing.JButton();
        jLabel6 = new javax.swing.JLabel();
        jButton2 = new javax.swing.JButton();
        jLabel7 = new javax.swing.JLabel();
        jTabbedPane1 = new javax.swing.JTabbedPane();
        jScrollPane3 = new javax.swing.JScrollPane();
        tbBusinessApply = new javax.swing.JTable();
        jScrollPane5 = new javax.swing.JScrollPane();
        tbBusinessRunning = new javax.swing.JTable();

        setLayout(new java.awt.BorderLayout());

        plSites.setBorder(javax.swing.BorderFactory.createTitledBorder(null, "各站点连接情况", javax.swing.border.TitledBorder.CENTER, javax.swing.border.TitledBorder.DEFAULT_POSITION));
        plSites.setPreferredSize(new java.awt.Dimension(250, 100));
        plSites.setLayout(new java.awt.FlowLayout(java.awt.FlowLayout.CENTER, 30, 50));
        add(plSites, java.awt.BorderLayout.LINE_START);

        jPanel3.setBorder(javax.swing.BorderFactory.createTitledBorder(null, "与各站点通信", javax.swing.border.TitledBorder.CENTER, javax.swing.border.TitledBorder.DEFAULT_POSITION));
        jPanel3.setPreferredSize(new java.awt.Dimension(400, 538));
        jPanel3.setLayout(new java.awt.BorderLayout());

        jPanel2.setLayout(new java.awt.BorderLayout());

        jScrollPane1.setBorder(javax.swing.BorderFactory.createTitledBorder(null, "消息记录", javax.swing.border.TitledBorder.CENTER, javax.swing.border.TitledBorder.DEFAULT_POSITION));

        taMessageRecorded.setEditable(false);
        taMessageRecorded.setColumns(20);
        taMessageRecorded.setLineWrap(true);
        taMessageRecorded.setRows(5);
        jScrollPane1.setViewportView(taMessageRecorded);

        jPanel2.add(jScrollPane1, java.awt.BorderLayout.CENTER);

        jScrollPane2.setBorder(javax.swing.BorderFactory.createTitledBorder(null, "发送消息", javax.swing.border.TitledBorder.CENTER, javax.swing.border.TitledBorder.DEFAULT_POSITION));

        taMessageSending.setColumns(20);
        taMessageSending.setLineWrap(true);
        taMessageSending.setRows(5);
        jScrollPane2.setViewportView(taMessageSending);

        jPanel2.add(jScrollPane2, java.awt.BorderLayout.SOUTH);

        jPanel3.add(jPanel2, java.awt.BorderLayout.CENTER);

        plSitesConnected.setBorder(javax.swing.BorderFactory.createTitledBorder(""));
        plSitesConnected.setAutoscrolls(true);
        plSitesConnected.setPreferredSize(new java.awt.Dimension(388, 200));
        java.awt.FlowLayout flowLayout1 = new java.awt.FlowLayout(java.awt.FlowLayout.CENTER, 10, 10);
        flowLayout1.setAlignOnBaseline(true);
        plSitesConnected.setLayout(flowLayout1);
        jPanel3.add(plSitesConnected, java.awt.BorderLayout.PAGE_START);

        jPanel8.setLayout(new java.awt.GridLayout(1, 0, 5, 0));
        jPanel8.add(jLabel5);

        jButton1.setText("发送");
        jButton1.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                jButton1ActionPerformed(evt);
            }
        });
        jPanel8.add(jButton1);
        jPanel8.add(jLabel6);

        jButton2.setText("清空");
        jButton2.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                jButton2ActionPerformed(evt);
            }
        });
        jPanel8.add(jButton2);
        jPanel8.add(jLabel7);

        jPanel3.add(jPanel8, java.awt.BorderLayout.PAGE_END);

        add(jPanel3, java.awt.BorderLayout.EAST);

        jTabbedPane1.setBorder(javax.swing.BorderFactory.createTitledBorder(null, "业务情况", javax.swing.border.TitledBorder.CENTER, javax.swing.border.TitledBorder.DEFAULT_POSITION));

        tbBusinessApply.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
                {null, null, null, null, null, null, null},
                {null, null, null, null, null, null, null},
                {null, null, null, null, null, null, null},
                {null, null, null, null, null, null, null}
            },
            new String [] {
                "序号", "主叫站号", "被叫站号", "申请业务状态", "主叫优先级", "所需信道资源", "申请批准情况"
            }
        ));
        tbBusinessApply.setRowHeight(30);
        jScrollPane3.setViewportView(tbBusinessApply);

        jTabbedPane1.addTab("各站点业务申请列表", jScrollPane3);

        tbBusinessRunning.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {
                {null, null, null, null},
                {null, null, null, null},
                {null, null, null, null},
                {null, null, null, null}
            },
            new String [] {
                "Title 1", "Title 2", "Title 3", "Title 4"
            }
        ));
        tbBusinessRunning.setRowHeight(30);
        jScrollPane5.setViewportView(tbBusinessRunning);

        jTabbedPane1.addTab("各站点业务运行情况", jScrollPane5);

        add(jTabbedPane1, java.awt.BorderLayout.CENTER);
    }// </editor-fold>//GEN-END:initComponents

    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        // TODO add your handling code here:
        this.taMessageSending.setText("");
    }//GEN-LAST:event_jButton2ActionPerformed

    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        // TODO add your handling code here:
        String str = this.taMessageSending.getText();
        if (!str.equals("")) {
            this.communicationServer.sendMessage(str);
            this.taMessageRecorded.append("[服务器]:" + this.taMessageSending.getText() + "\n");
            this.taMessageSending.setText("");
        }
    }//GEN-LAST:event_jButton1ActionPerformed
    // Variables declaration - do not modify//GEN-BEGIN:variables
    private javax.swing.JButton jButton1;
    private javax.swing.JButton jButton2;
    private javax.swing.JLabel jLabel5;
    private javax.swing.JLabel jLabel6;
    private javax.swing.JLabel jLabel7;
    private javax.swing.JPanel jPanel2;
    private javax.swing.JPanel jPanel3;
    private javax.swing.JPanel jPanel8;
    private javax.swing.JScrollPane jScrollPane1;
    private javax.swing.JScrollPane jScrollPane2;
    private javax.swing.JScrollPane jScrollPane3;
    private javax.swing.JScrollPane jScrollPane5;
    private javax.swing.JTabbedPane jTabbedPane1;
    private javax.swing.JPanel plSites;
    private javax.swing.JPanel plSitesConnected;
    private javax.swing.JTextArea taMessageRecorded;
    private javax.swing.JTextArea taMessageSending;
    private javax.swing.JTable tbBusinessApply;
    private javax.swing.JTable tbBusinessRunning;
    // End of variables declaration//GEN-END:variables

    public Constants getConstants() {
        return this.constant;
    }

    public ArrayList<JLabel> getSiteLabels() {
        return this.siteLabels;
    }

    public JPanel getSiteConnectedPanel() {
        return this.plSitesConnected;
    }

    public ArrayList<JCheckBox> getSiteCheckBoxs() {
        return this.siteCheckBoxes;
    }

    public JTextArea getMessageRecordTextArea() {
        return this.taMessageRecorded;
    }

    public JTextArea getMessageSendingTextArea() {
        return this.taMessageSending;
    }

    public JTable getTableApply() {
        return tbBusinessApply;
    }

    public JTable getTableRunning() {
        return tbBusinessRunning;
    }

    public BusinessApplyTableModel getApplyModel() {
        return businessApplyTM;
    }

    public BusinessRunningTableModel getRunningModel() {
        return businessRunningTM;
    }

    public void updateSnr(String id, String snr){
        businessRunningTM.updateSnr(id, snr);
    }
    
//    public void bizCancelling(String idCalling) {
//        final String id = idCalling;
//        SwingUtilities.invokeLater(new Runnable() {
//            @Override
//            public void run() {
//                if (JOptionPane.OK_OPTION == JOptionPane.showConfirmDialog(null, "站点" + id + "申请取消业务")) {
//                    Object idCalled = getRunningModel().getRelativeSite(id);
//                    if (idCalled != null) {
//                        communicationServer.sendCancelPermission(id, idCalled.toString());
//                        System.out.println("not null");
//                    }
//                }
//            }
//        });
//    }
}
